    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            Temporada {cycle.season} · {cycle.crop.lot.area} ha · Portainjerto {cycle.crop.rootstock ?? "—"}
          </p>
        </div>
        {dirty && (
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-sm font-semibold text-gray-700 flex items-center gap-2">
            <Leaf className="w-4 h-4 text-green-500" /> Estado fenológico
          </CardTitle>
        </CardHeader>
        <CardContent>
          ...
          <p className="text-xs text-gray-400 mt-2">
            Actual: <strong className="text-gray-700">{ESTADO_FENOLOGICO_LABELS[cycle.estadoFenologico]}</strong>
            {" · "}Haz clic en un estado para actualizar
          </p>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-semibold text-gray-700 flex items-center gap-2">
              <Snowflake className="w-4 h-4 text-blue-400" /> Horas frío acumuladas
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="flex items-end gap-3">
              ...
              <span className="text-gray-400 text-sm pb-2">/ {UMBRALES.HORAS_FRIO_META}h meta</span>
            </div>
            ...
            <p className="text-xs text-gray-400">
              {horasPct}% · {cycle.horasFrioAcumuladas >= UMBRALES.HORAS_FRIO_META
                ? "✅ Meta cumplida"
                : `Faltan ${UMBRALES.HORAS_FRIO_META - cycle.horasFrioAcumuladas}h`}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-semibold text-gray-700 flex items-center gap-2">
              <Scale className="w-4 h-4 text-purple-400" /> Estimaciones de cosecha
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label className="text-xs text-gray-400">Calibre estimado (mm)</Label>
                <Input
                  type="number"
                  step="0.1"
              ...
              <div>
                <Label className="text-xs text-gray-400">Rendimiento est. (kg/ha)</Label>
                <Input
                  type="number"
                  className="mt-1 h-8 text-sm"
            ...
            <div>
              <Label className="text-xs text-gray-400 flex items-center gap-1">
                <Calendar className="w-3 h-3" /> Fecha cosecha estimada
              </Label>
              <Input
                type="date"
                className="mt-1 h-8 text-sm"
            ...
            <div>
              <Label className="text-xs text-gray-400">Destino producción</Label>
              <Select
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-sm font-semibold text-gray-700 flex items-center gap-2">
            <TrendingUp className="w-4 h-4 text-emerald-500" /> Resumen financiero del ciclo
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            <div>
              <div className="text-xs text-gray-400">Insumos</div>
              <div className="font-bold text-gray-800">${(totalInsumos / 1_000_000).toFixed(3)}M</div>
              <div className="text-xs text-gray-400">{cycle.inputs.length} reg.</div>
            </div>
            <div>
              ...
              <div className="text-xs text-gray-400">{cycle.laborRecords.length} jornadas</div>
            </div>
            <div>
            ...
            {ingresosEst && (
              <div>
                ...
                <div className="text-xs text-green-500">
                  Margen: ${((ingresosEst - totalCost) / 1_000_000).toFixed(2)}M
                </div>
              </div>
            )}
          </div>
          ...
          {cycle.notas && (
            <>
              ...
              <div className="text-xs text-gray-500">
                <span className="font-medium text-gray-600">Notas: </span>{cycle.notas}
              </div>
            </>
          )}
        </CardContent>
